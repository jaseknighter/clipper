...
function sample_player.load_file(file)
  ...
  if file ~= "cancel" then
    file_selected = true
    sample_player.nav_labels[1] = "select/play sample"
...
function sample_player.autogenerate_cutters(a)
  if waveform_loaded and sample_player.nav_active_control > 1 then
...
function sample_player.draw_sub_nav ()
  ...
  screen.fill()
  screen.level(0)
  local num_field_menu_areas = #sample_player.nav_labels
...
function sample_player.draw_top_nav (msg)
  subnav_title = sample_player.nav_labels[sample_player.nav_active_control] 
  if msg == nil then
    if sample_player.nav_active_control == 2 then
      subnav_title = subnav_title .. ": " .. sample_player.play_mode_text[play_mode+1]
